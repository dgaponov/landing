...
- Support pour les événements, marqueurs, axes et grille
- Regroupement intelligent des marqueurs avec zoom automatique vers le groupe - Cliquez sur les marqueurs groupés pour zoomer sur leurs composants individuels
- Rendu virtualisé pour une meilleure performance avec de grands ensembles de données (activé uniquement lorsque le contenu de la chronologie dépasse la zone visible)
  ...
  const { timeline } = useTimeline({
    ...
    viewConfiguration: {
      markers: {
        ...
        groupZoomPadding: 0.3,        // Rembourrage de 30 % autour du groupe
...
La chronologie est implémentée en tant que composant React qui peut être configuré via deux objets principaux :
...
- [Timeline basique](https://preview.gravity-ui.com/timeline/?path=/story/timeline-events--basic) - Timeline simple avec événements et axes
