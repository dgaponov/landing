...
2. Connectez le plugin à Webpack (exemple pour `@gravity-ui/app-builder`) :
    ...
    // Par exemple. Lisez tous les fichiers avec les textes localisés et stockez-les dans cette cartographie
    const locales = {
        en: {},
        ru: {},
        tr: {},
    ...
    // En utilisant applyPluginToWebpackConfig, le plugin WebpackAssetsManifest sera également connecté,
...
3. Configurez les statiques dynamiques à partir du manifest d'assets sur le serveur (exemple avec `@gravity-ui/app-layout`) :
...
La signature est similaire à l'`importSpecifier` original de webpack.
...
const importResolver = (source: string, exportName: string, _identifierName: string, module: string) => {
    ...
    // Traitement de l'import d'une fonction d'aide et spécification qu'elle appartient à un keyset commun (namespace).
...
Pour assurer un fonctionnement correct, il est toujours nécessaire de retourner un format détaillé dans la fonction `Replacer`. Cela est important car, lors du remplacement, il existe une possibilité de modifier les clés et les ensembles de clés déterminés automatiquement.
