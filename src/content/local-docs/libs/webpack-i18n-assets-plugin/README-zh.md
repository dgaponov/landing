...
Webpack 插件，用于将本地化函数（i18n）的调用替换为目标文本。
...
启用项目中收集未使用键的模式。构建完成后，会创建一个名为 `unused-keys.json` 的文件。
...
在实现这个插件时，我们借鉴了 webpack-localize-assets-plugin 包中的一个想法（非常感谢该包的作者！）。

主要区别如下：

- 更便捷的 API，支持处理各种国际化函数（包括 i18next 中的 useTranslation 等命名空间助手、从其他模块导入的函数等）
- 正确生成相对于源代码的源映射
