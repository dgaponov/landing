...
2. 플러그인을 Webpack에 연결하세요 (@gravity-ui/app-builder 예시):
    ...
    // 예시. 지역화된 텍스트가 포함된 모든 파일을 읽고 이 매핑에 저장
    const locales = {
        en: {},
        ru: {},
        tr: {},
    ...
    // applyPluginToWebpackConfig를 사용할 때 WebpackAssetsManifest 플러그인도 연결되어 각 로케일에 대한 에셋 매니페스트를 생성합니다.
    module.exports = applyPluginToWebpackConfig(webpackConfig, {locales});
    ```

    @gravity-ui/app-builder를 사용하는 경우 예시:
...
const importResolver = (source: string, exportName: string, _identifierName: string, module: string) => {
    // 특정 경로에 기반한 모듈 처리를 무시해야 하는 경우 이처럼 처리할 수 있습니다.
...
### [webpack-localize-assets-plugin](https://github.com/privatenumber/webpack-localize-assets-plugin)와 비교했을 때 어떻게 다를까요?

이 플러그인을 구현하기 위해 webpack-localize-assets-plugin 패키지의 아이디어를 사용했습니다 (패키지 제작자에게 큰 감사를 드립니다!).

다음과 같은 차이점이 있습니다:

- 모든 종류의 국제화 함수(예: i18next의 useTranslation 같은 네임스페이스 헬퍼, 다른 모듈에서 가져온 함수 등)와 함께 사용할 수 있는 더 편리한 API
