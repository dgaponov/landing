...
Die Bibliothek dient dazu, Widgets in einem Gitter anzuordnen, sie zu vergrößern, neue hinzuzufügen und zu löschen.
Ein Widget ist eine React-Komponente. Zum Beispiel Text, Grafiken oder Bilder.
...
- **overlayControls**: Ein Objekt, das die Widget-Steuerelemente während der Bearbeitung überschreibt. Wenn es nicht übermittelt wird, werden die grundlegenden Steuerelemente angezeigt. Wenn `null` übergeben wird, wird nur der Schließen-Button oder ein benutzerdefiniertes Menü angezeigt.
...
- **getPreparedCopyItemOptions**: Wird aufgerufen, um ein kopiertes Element in ein serialisierbares Objekt umzuwandeln, bevor es in localStorage gespeichert wird. Es sollte anstelle der veralteten Prop `context.getPreparedCopyItemOptions` verwendet werden.
...
export interface Config {
  ...
  layout: ConfigLayout[]; // Widget-Positionen im Gitter https://github.com/react-grid-layout
...
Ein neues Element zur Konfiguration hinzufügen:
...
Ein bestehendes Element in der Konfiguration ändern:
...
Ein Element aus der Konfiguration löschen:
...
`DashKit` erzeugt Parameter basierend auf den Standardparametern für Widgets, Links und Aliase. Diese Parameter sind für die [ChartKit](https://github.com/gravity-ui/chartkit)-Bibliothek erforderlich.
...
- **dragImageSrc**: Vorschau-Bild für den Drag-Vorgang, standardmäßig wird ein transparentes 1px-PNG-Base64 verwendet
- **onDragStart**: Callback, der aufgerufen wird, wenn ein Element aus dem ActionPanel gezogen wird
- **onDragEnd**: Callback, der aufgerufen wird, wenn ein Element abgelegt oder der Drag-Vorgang abgebrochen wird
```
...
const onDrop = (dropProps: ItemDropProps) => {
  // ... füge das Element zu deiner Konfiguration hinzu
  dropProps.commit();
}
...
Neue Änderungen aus dem Projekt werden nicht immer erkannt, wenn Storybook läuft, daher ist es besser, das Projekt manuell neu zu bauen und Storybook neu zu starten.
