...
이 라이브러리는 위젯을 그리드에 배치하고, 크기를 조정하며, 새 위젯을 추가하거나 삭제하는 데 사용됩니다.
...
- **onDrag**: ReactGridLayout에서 아이템 드래그 중에 호출됩니다.
- **onDragStop**: ReactGridLayout에서 아이템 드래그가 종료될 때 호출됩니다.
- **onResizeStart**: ReactGridLayout에서 아이템 크기 조정이 시작될 때 호출됩니다.
- **onResize**: ReactGridLayout에서 아이템 크기 조정 중에 호출됩니다.
- **onResizeStop**: ReactGridLayout에서 아이템 크기 조정이 종료될 때 호출됩니다.
...
- **onCopyFulfill**: 아이템 복사가 완료될 때 호출됩니다. 성공 시 `error=null`과 정의된 `data`와 함께, 실패 시 `error: Error`와 `data` 없이 호출됩니다.
...
- **dragImageSrc**: 드래그 이미지 미리보기, 기본적으로 투명한 1px PNG base64 사용
- **onDragStart**: ActionPanel에서 요소가 드래그될 때 호출되는 콜백
