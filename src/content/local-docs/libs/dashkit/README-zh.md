...
该库用于在网格中排列小部件、调整它们的大小、添加新的小部件并删除它们。
小部件是一个 React 组件，例如文本、图形和图像。
...
- **onChange**：当 config 或 [itemsStateAndParams](#itemsStateAndParams) 发生变化时调用。
- **onDrop**：使用 (#DashKitDnDWrapper) 从 ActionPanel 拖放小部件时调用。
- **onItemMountChange**：小部件挂载状态发生变化时调用。
- **onItemRender**：小部件渲染完成时调用。
- **defaultGlobalParams**、**globalParams**：影响所有小部件的 [参数](#Params)。在 DataLens 中，`defaultGlobalParams` 是仪表板设置中设置的全局参数。`globalParams` 是可以通过 URL 设置的全局参数。
- **itemsStateAndParams**： [itemsStateAndParams](#itemsStateAndParams)。
- **settings**：DashKit 设置。
- **context**：将传递给所有小部件的对象。
- **overlayControls**：在编辑时覆盖小部件控制的对象。如果未传递，将显示基本控制。如果传递 `null`，则仅显示关闭按钮或自定义菜单。
- **overlayMenuItems**：自定义下拉菜单项。
- **noOverlay**：如果为 `true`，则在编辑时不显示覆盖层和控制。
- **focusable**：如果为 `true`，则网格小部件可聚焦。
...
- **getPreparedCopyItemOptions**：在将复制的小部件转换为可序列化对象并保存到 localStorage 之前调用。它应替代已弃用的 `context.getPreparedCopyItemOptions` 属性使用。
...
在使用 `DashKit` 作为 React 组件之前，必须先对其进行配置。
...
type ItemDropProps = {
    ...
    itemLayout: ConfigLayout; // 计算得到的项布局尺寸
    newLayout: ConfigLayout[]; // 元素放置后新的布局
};
```
...
| `--dashkit-action-panel-border-color`          | 边框色         |
| `--dashkit-action-panel-border-radius`         | 边框圆角       |
| 操作面板项变量                                 |                |
| `--dashkit-action-panel-item-color`            | 背景色         |
| `--dashkit-action-panel-item-text-color`       | 文本色         |
| `--dashkit-action-panel-item-color-hover`      | 悬停背景色     |
| `--dashkit-action-panel-item-text-color-hover` | 悬停文本色     |
| 覆盖层变量                                     |                |
| `--dashkit-overlay-border-color`               | 边框色         |
