...
Виджет — это компонент React. Например, текст, графики или изображения.
...
- **onCopyFulfill**: Вызывается после завершения копирования элемента: с `error=null` и определённым `data` при успешной операции или с `error: Error` без `data` в противном случае.

## Использование

### Конфигурация DashKit

Перед использованием `DashKit` как компонента React его необходимо настроить.
