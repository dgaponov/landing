...
- `BaseTable` – eine Komponente mit grundlegenden Stilen;
...
Verwenden Sie dies, wenn Sie den Grid-Container als Scroll-Element nutzen möchten (wenn Sie das Fenster verwenden möchten, siehe Abschnitt zur Fenster-Virtualisierung). Stellen Sie sicher, dass Sie eine feste Höhe für den Container festlegen; andernfalls funktioniert die Virtualisierung nicht.
...
Wenn Sie Virtualisierung zusammen mit der Reordering-Funktion verwenden, müssen Sie auch die Option `rangeExtractor` übergeben:
...
Erfahren Sie mehr über die Tabelle und die Eigenschaften zur Spaltengrößenanpassung in der react-table [Dokumentation](https://tanstack.com/table/v8/docs/api/features/column-sizing)