...
复数化可用于轻松本地化依赖数值的键。当前库通过 [Intl.PluralRules API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) 使用 [CLDR Plural Rules](https://unicode-org.github.io/cldr-staging/charts/latest/supplemental/language_plural_rules.html)。
...
复数化键包含 4 个值，每个值对应一个 `PluralForm` 枚举值。枚举值为：`One`、`Few`、`Many` 和 `None`，分别对应。复数化的模板变量名为 `count`。
...
准备一个 JSON 键集文件，以便类型化过程能够获取数据。在你从哪里获取键集的地方，添加创建额外 `data.json` 文件的逻辑。为了减少文件大小并加速 IDE 解析，你可以将所有值替换为 `'str'`。
...
实现嵌套结构遍历和条件类型的内置函数方法是一个足够复杂任务。因此，类型化仅在使用直接函数调用和最多三个参数的 `bind` 调用时有效。
