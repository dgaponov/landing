...
Acepta un objeto `options` con un `logger` opcional que se usará para registrar advertencias de la biblioteca.
 ...
 * `options` - objeto de opciones de registro:
   ...
   * `extra` - objeto de opciones adicionales, con una sola cadena `type`, que siempre es `i18n`
...
La biblioteca soporta plantillas. Las variables con plantillas se encierran en llaves dobles, y los valores se pasan a la función i18n como un diccionario clave-valor:
...
La pluralización se puede usar para una localización sencilla de claves que dependen de valores numéricos. La biblioteca actual utiliza las [Reglas de Plural de CLDR](https://unicode-org.github.io/cldr-staging/charts/latest/supplemental/language_plural_rules.html) a través de la [API Intl.PluralRules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules).

Es posible que necesites [polyfill](https://github.com/eemeli/intl-pluralrules) la [API Intl.PluralRules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) si no está disponible en el navegador.
...
- other (forma requerida para todos los idiomas — forma plural general — también se usa si el idioma solo tiene una forma única)
...
Una clave pluralizada contiene 4 valores, cada uno correspondiente a un valor del enum `PluralForm`. Los valores del enum son: `One`, `Few`, `Many` y `None`, respectivamente. El nombre de la variable de plantilla para la pluralización es `count`.
...
Una función de pluralización acepta un número y el enum `PluralForm`, y se espera que devuelva uno de los valores del enum dependiendo del número proporcionado.
...
Los dos idiomas compatibles de forma nativa son el inglés y el ruso.
...
El conjunto de reglas en inglés se usa por defecto, para cualquier idioma sin una función de pluralización configurada.
...
Profundidad máxima de anidamiento limitada: solo 1 nivel (para glosario)
</span>

El anidamiento te permite referenciar otras claves en una traducción. Puede ser útil para construir términos de glosario.
...
Puedes referenciar claves de otros conjuntos de claves preponiendo el nombre del conjunto: 
```json
// global/en.json
{
  "app": "App"
...
En tus directorios `ui/utils/i18n` (donde configuras i18n y lo exportas para que sea usado por todas las interfaces), importa la función de tipado `I18NFn` con tus `Keysets`. Después de que tu i18n haya sido configurado, devuelve la función casteada
...
**Por qué el tipado a través de una clase no está soportado**

Esta función puede romper o complicar algunos escenarios de i18n, por lo que se agregó como una extensión funcional. Si resulta efectiva, probablemente la agregaremos a una clase para evitar el casteo de las funciones exportadas.

**Por qué los métodos integrados podrían fallar**

La implementación de recorrido de estructuras anidadas y tipos condicionales usando métodos de funciones integradas tipadas es una tarea lo suficientemente compleja. Por eso, el tipado funciona solo cuando se usa una llamada directa a la función y una llamada a `bind` hasta el tercer argumento.

**¿Por qué no puedo generar un archivo ts directamente para castear los valores de las claves también?**

Puedes hacerlo pasando el tipo de resultado a I18NFn. Sin embargo, con tamaños de archivo grandes, ts comienza a consumir cantidades enormes de recursos, ralentizando el IDE drásticamente, pero con un archivo JSON esto no ocurre.

**¿Por qué otros métodos de la clase I18N no han sido tipados?**

Pueden ser tipados, apreciamos si ayudas a implementarlo. El caso es que otros métodos se usan en el 1% de los casos.