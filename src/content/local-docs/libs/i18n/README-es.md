...
Acepta un objeto `options` con un `logger` opcional que se usaría para registrar advertencias de la biblioteca.

##### logger

El logger debe tener un método `log` explícito con la siguiente firma:

 * `message` - cadena del mensaje que se registrará
...
La biblioteca soporta plantillas. Las variables con plantillas están encerradas en llaves dobles, y los valores se pasan a la función i18n como un diccionario clave-valor:
...
La pluralización se puede usar para una localización sencilla de claves que dependen de valores numéricos. La biblioteca actual utiliza las [Reglas de Plural CLDR](https://unicode-org.github.io/cldr-staging/charts/latest/supplemental/language_plural_rules.html) a través de la [API Intl.PluralRules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules).

Es posible que necesites un [polyfill](https://github.com/eemeli/intl-pluralrules) para la [API Intl.PluralRules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) si no está disponible en el navegador.
...
Los dos idiomas soportados de fábrica son el inglés y el ruso.
...
Puedes hacer referencia a claves de otros conjuntos de claves preponiendo el nombre del conjunto de claves: 
```json
// global/en.json
{
  "app": "App"
...
Esta función puede romper o complicar algunos escenarios de i18n, por lo que se agregó como una extensión funcional. Si resulta efectiva, probablemente la agreguemos a una clase para evitar el casteo de funciones exportadas.

**Por qué los métodos integrados podrían fallar**

Implementar el recorrido de estructuras anidadas y tipos condicionales usando métodos de funciones integradas tipadas es una tarea lo suficientemente compleja. Por eso, el tipado funciona solo cuando se usa una llamada directa a la función y una llamada a `bind` hasta el tercer argumento.
...
Se pueden tipar, apreciamos si ayudas a implementarlo. El caso es que otros métodos se usan en el 1% de los casos.