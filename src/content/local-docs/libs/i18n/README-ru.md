...
Библиотека поддерживает шаблонизацию. Переменные шаблона заключаются в двойные фигурные скобки, а их значения передаются в функцию i18n в виде словаря ключ-значение:
...
Если API недоступен в браузере, может потребоваться [полифилл](https://github.com/eemeli/intl-pluralrules) для [Intl.PluralRules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules).
...
- few (множественное малое число)
- many (множественное большое число; также используется для дробей, если они имеют отдельный класс)
...
#### [Устаревший] Старый формат плюрализации
...
Плюрализованный ключ содержит 4 значения, каждое из которых соответствует значению перечисления `PluralForm`. Значения перечисления: `One`, `Few`, `Many` и `None` соответственно. Имя переменной шаблона для плюрализации — `count`.

#### [Устаревший] Пользовательская плюрализация

Поскольку в каждом языке своя система плюрализации, библиотека предоставляет метод для настройки правил для любого выбранного языка.
...
Подготовьте файл набора ключей в формате JSON, чтобы процедура типизации могла получить данные. Там, где вы загружаете наборы ключей, добавьте создание дополнительного файла `data.json`. Чтобы уменьшить размер файла и ускорить разбор в IDE, вы можете заменить все значения на `'str'`.
...
В ваших директориях `ui/utils/i18n` (где вы настраиваете i18n и экспортируете его для использования во всех интерфейсах) импортируйте функцию типизации `I18NFn` с вашими `Keysets`. После настройки i18n верните приведённую функцию
...
Эта функция может нарушить или усложнить некоторые сценарии i18n, поэтому она была добавлена как функциональное расширение. Если она окажется эффективной, мы, вероятно, добавим её в класс, чтобы избежать приведения экспортируемых функций.

**Почему встроенные методы могут не работать**

Реализация обхода вложенных структур и условных типов с использованием типизированных встроенных методов функций — это достаточно сложная задача. Поэтому типизация работает только при прямом вызове функции и вызове `bind` до третьего аргумента.

**Почему нельзя просто сгенерировать ts-файл для приведения типов значений ключей?**

Это можно сделать, передав результирующий тип в I18NFn. Однако при больших размерах файлов ts начинает потреблять огромные ресурсы, резко замедляя IDE, но с JSON-файлом этого не происходит.

**Почему другие методы класса I18N не типизированы?**

Их можно типизировать, мы будем признательны за помощь в реализации. Дело в том, что другие методы используются лишь в 1% случаев.