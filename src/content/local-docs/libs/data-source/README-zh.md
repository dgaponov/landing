...
**Data Source** 是一个围绕数据获取的简单包装器。它是 [clean architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html) 中的一种“端口”。它允许您根据具体使用场景，为数据获取相关的逻辑创建自定义包装器。**Data Source** 在底层使用了 [react-query](https://tanstack.com/query/latest)。
...
基于默认的 `DataLoader` 编写一个组件，以定义加载状态和错误的显示方式：
...
这在条件查询中特别有用，当您只想在特定条件下加载数据时，同时保持类型安全。
