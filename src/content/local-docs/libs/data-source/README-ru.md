...
**Data Source** — это простая обертка над получением данных. Это своего рода «порт» в [clean architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html). Она позволяет создавать обертки вокруг логики получения данных в зависимости от ваших сценариев использования. **Data Source** использует [react-query](https://tanstack.com/query/latest) под капотом.
...
- `loading` — фактическая загрузка данных. То же самое, что и `isLoading` в React Query
...
1. **Type Safety** — TypeScript правильно выводит типы для условных параметров
2. **Performance** — избегает ненужных запросов к серверу
3. **Logic Simplicity** — не нужно управлять дополнительным состоянием `enabled`
4. **Consistency** — единый подход для всех условных запросов
...
- `fetch` - Функция, выполняющая фактическую загрузку данных
...
Создает источник данных для бесконечных запросов, подходящий для пагинации и бесконечной прокрутки.
...
Основной хук для загрузки данных с использованием источника данных.
...
- `refetch` - Функция для обновления всех запросов
- `refetchErrored` - Функция для обновления только неудачных запросов

#### `useRefetchAll(states)`

Создает колбэк для обновления нескольких запросов.

```ts
const refetchAll = useRefetchAll([user, posts, comments]);
// refetchAll() запустит обновление для всех запросов
```

#### `useRefetchErrored(states)`

Создает колбэк для обновления только неудачных запросов.

```ts
const refetchErrored = useRefetchErrored([user, posts, comments]);
// refetchErrored() обновит только запросы с ошибками
```
...
Возвращает контекст запроса (для создания кастомных хуков данных на базе react-query).
