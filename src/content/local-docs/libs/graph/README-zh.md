...
> [从 0.x 到 1.x 的迁移指南 →](docs/migration-guides/v0-to-v1.md)

一个图可视化库，融合了两者的优点：
- Canvas 用于查看完整图时的高性能渲染
- HTML/React 用于放大时的丰富交互

无需在性能和交互性之间做出选择。非常适合大型图表、流程图以及基于节点的编辑器。

## 动机

现代 Web 应用常常需要复杂的可视化和交互功能，但现有的解决方案通常只专注于单一的渲染技术：

- **Canvas** 在复杂图形方面提供高性能，但文本处理和交互性有限。
- **HTML DOM** 便于构建界面，但处理复杂图形或大量元素时效率较低。
...
该库采用智能渲染系统，自动管理 Canvas 和 React 组件之间的切换：

1. 在低缩放级别时，一切都在 Canvas 上渲染以确保性能
2. 当放大到详细视图时，`GraphCanvas` 组件会：
   - 跟踪相机视口和缩放变化
...
2. 组件
   - [画布图组件](docs/components/canvas-graph-component.md)
