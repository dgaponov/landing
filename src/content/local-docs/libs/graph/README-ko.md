...
더 이상 성능과 상호작용성 사이에서 선택할 필요가 없습니다. 대형 다이어그램, 플로우차트, 노드 기반 에디터에 완벽합니다.

## 동기

현대 웹 애플리케이션은 종종 복잡한 시각화와 상호작용을 요구하지만, 기존 솔루션은 일반적으로 단일 렌더링 기술에 초점을 맞춥니다:

- **Canvas**는 복잡한 그래픽에 높은 성능을 제공하지만, 텍스트 처리와 상호작용 측면에서 제한적입니다.
- **HTML DOM**은 인터페이스에 편리하지만, 복잡한 그래픽이나 대량의 요소를 다룰 때는 효율성이 떨어집니다.

@gravity-ui/graph는 확대 수준에 따라 Canvas와 HTML 간을 자동으로 전환하여 이 문제를 해결합니다:
- **축소 상태**: 전체 그래프의 효율적인 렌더링을 위해 Canvas 사용
- **중간 확대**: 기본 상호작용이 포함된 개략도 뷰 표시
- **확대 상태**: 풍부한 상호작용을 위해 HTML/React 컴포넌트로 전환

## 작동 원리

이 라이브러리는 Canvas와 React 컴포넌트 간의 전환을 자동으로 관리하는 스마트 렌더링 시스템을 사용합니다:

1. 낮은 확대 수준에서는 성능을 위해 모든 요소를 Canvas에 렌더링합니다.
2. 세부 뷰로 확대할 때 `GraphCanvas` 컴포넌트는:
   - 카메라 뷰포트와 스케일 변경을 추적합니다.
   - 부드러운 스크롤을 위한 패딩을 포함하여 현재 뷰포트에 보이는 블록을 계산합니다.
   - 보이는 블록에 대해서만 React 컴포넌트를 렌더링합니다.
   - 스크롤이나 확대 시 목록을 자동으로 업데이트합니다.
   - 축소할 때 React 컴포넌트를 제거합니다.
