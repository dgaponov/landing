...
YAML 구성을 사용해 [@gravity-ui/page-constructor](https://github.com/gravity-ui/page-constructor) 패키지를 통해 정적 페이지를 빌드하는 강력한 명령줄 유틸리티입니다. 구성 세부 사항은 [page-constructor storybook](https://preview.gravity-ui.com/page-constructor/)을 참조하세요.
...
- `-i, --input <path>`: YAML 파일이 포함된 입력 디렉토리 (기본값: "./pages")
...
페이지 생성 빌더는 이미지, 아이콘 및 기타 파일과 같은 정적 자산을 자동으로 처리합니다. 구성 파일에서 자산 디렉토리를 설정하세요:

```yaml
# page-builder.config.yml
input: ./pages
output: ./dist
assets: ./assets # 복사할 자산 디렉토리
```
...
페이지 생성 빌더는 정적 페이지에 파비콘을 추가하는 것을 지원합니다. 자산 디렉토리의 로컬 파일이나 외부 URL을 지정할 수 있습니다.
...
```yaml
# page-builder.config.yml
favicon: logo.svg # 자산 디렉토리의 로컬 파일
# 또는
...
프로젝트 루트에 `navigation.yml` 파일을 생성하거나(또는 구성에서 사용자 지정 경로를 지정) 다음과 같이 설정하세요:
...
특정 페이지에 대해 네비게이션을 재정의하려면 페이지 YAML에 `navigation` 섹션을 직접 추가할 수 있습니다:
