...
페이지 생성기 빌더는 이미지, 아이콘 및 기타 파일과 같은 정적 자산을 자동으로 처리합니다. 구성 파일에서 자산 디렉토리를 설정하세요:
...
페이지 생성기 빌더는 정적 페이지에 파비콘을 추가하는 것을 지원합니다. 자산 디렉토리의 로컬 파일이나 외부 URL을 지정할 수 있습니다.
...
CDN이나 다른 도메인에서 외부 favicon URL을 사용할 수도 있습니다:
...
페이지 생성기 빌더는 모든 페이지에 나타나는 전역 네비게이션 구성을 지원합니다. 네비게이션은 별도의 YAML 파일을 통해 구성됩니다.

#### 네비게이션 구성

프로젝트 루트에 `navigation.yml` 파일을 생성하세요 (또는 구성에서 사용자 지정 경로를 지정하세요):
