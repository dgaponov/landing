...
首先，我们需要一个 React 项目和某种服务器。例如，你可以使用 Vite 创建 React 项目并搭配 Express 服务器，或者直接创建 Next.js 应用——它同时具备客户端和服务器端。
...
这是一个最简单的集成示例。为了让 YFM 标记正常工作，你需要在服务器端处理内容，并在客户端接收它。

如果你的服务器是一个独立的应用程序，那么你需要安装 page-constructor：
...
要启动新项目，你可以使用我们准备的[基于 Next.js 的现成模板](https://github.com/gravity-ui/page-constructor-website-template)。

### 静态站点构建器

[Page Constructor Builder](https://github.com/gravity-ui/page-constructor-builder) - 一个命令行实用工具，用于从 YAML 配置构建静态页面，使用 @gravity-ui/page-constructor。
...
在底层，该包使用 `diplodoc/transform` 来将 Yandex 风格的 Markdown 转换为 HTML，因此它也被列为 peer 依赖。

您还可以在需要的地方使用这些有用的工具，例如在自定义组件中。
...
有关使用服务器工具的全面指南，包括详细解释和高级用例，请参阅[服务器工具使用附加章节](./docs/data-preparation.md)。

### 自定义块

页面构造器允许您在应用中使用用户定义的块。这些块是普通的 React 组件。
...
要添加自定义 `loadable` 块，请将 `custom.loadable` 属性传递给 `PageConstructor`，其中键为组件的数据源名称（字符串），值为一个对象。

```typescript
export interface LoadableConfigItem {
  fetch: FetchLoadableData; // 数据加载方法
  component: React.ComponentType; // 用于传递加载数据的组件
}
...
子块是可以在块 `children` 属性中使用的组件。在配置中，指定子块的子组件列表。渲染后，这些子块作为 `children` 传递给块。
...
7. 在块目录的 `__stories__` 文件夹中添加 Storybook 演示。故事的所有演示内容应放置在故事目录的 `data.json` 中。通用的 `Story` 必须接受块 props 的类型，否则 Storybook 中将显示不正确的块 props。
...
`page-constructor` 是一个基于 `uikit` 的库，我们使用 uikit 中的 `i18n` 实例。要设置国际化，您只需使用 uikit 中的 `configure`：
...
要开始使用任何分析，请将处理程序传递给构造器。处理程序必须在项目端创建。处理程序将接收 `default` 和 `custom` 事件对象。传递的处理程序将在按钮、链接、导航和控件点击时触发。由于一个处理程序用于所有事件处理，请在创建处理程序时注意如何处理不同事件。有一些预定义字段来帮助您构建复杂逻辑。
...
事件对象只有一个必需字段 - `name`。它还具有预定义字段，这些字段有助于管理复杂逻辑。例如，`counter.include` 可以帮助在项目中使用多个分析系统时，将事件发送到特定计数器。
...
几个预定义事件类型用于标记自动配置的事件。例如，可以使用这些类型来过滤默认事件。
...
当您的拉取请求获得代码所有者的批准并通过所有检查后，请执行以下操作：

1. 检查是否存在来自机器人的发布拉取请求，其中包含其他贡献者的变更（看起来像 `chore(main): release 0.0.0`）。如果存在，请检查为什么它未被合并。如果贡献者同意发布共享版本，请继续下一步。如果不同意，请让他发布他的版本，然后继续下一步。
...
4. 您还可以看到手动版本字段。如果这是您分支中的首次 alpha 发布，请不要在这里设置任何内容。首次发布后，您必须手动设置新版本，因为如果分支即将过期，我们不会更改 package.json。在手动版本中使用 `alpha` 前缀，否则会出错。
5. 点击“Run workflow”并等待操作完成。您可以发布任意多个版本，但请勿滥用，仅在真正需要时发布版本。在其他情况下，使用 [npm pack](https://docs.npmjs.com/cli/v7/commands/npm-pack)。

### Beta-major 版本发布

如果您想发布新 major 版本，在稳定版本之前可能需要 beta 版本，请执行以下操作：

1. 创建或更新分支 `beta`。
2. 在其中添加您的变更。
3. 当您准备好新 beta 版本时，使用空提交手动发布它（或者您可以将此带有页脚的提交消息添加到最后一个提交）：
...
4. 发布后，机器人将向分支 `beta` 创建一个新 PR，其中包含更新的 CHANGELOG.md 和包版本提升。
5. 您可以重复此操作任意多次。当您准备好发布最新的 major 版本（不带 beta 标签）时，您需要从分支 `beta` 创建 PR 到分支 `main`。请注意，您的包版本带有 beta 标签是正常的。机器人知道这一点并会正确更改它。`3.0.0-beta.0` 将变为 `3.0.0`。

### 先前 major 版本的发布流程

如果您想在将提交到 main 后，在先前 major 中发布新版本，请执行以下操作：

1. 更新必要的分支，先前 major 发布分支名称为：
   1. `version-1.x.x/fixes` - 用于 major 1.x.x
   2. `version-2.x.x` - 用于 major 2.x.x
2. 从先前 major 发布分支检出新分支。
3. 从分支 `main` 樱桃挑选您的提交。
4. 创建 PR，获得批准并合并到先前 major 发布分支。
...
这个项目包含一个全面的 **Memory Bank** - 这是一个 Markdown 文档文件集合，提供关于项目架构、组件和使用模式的详细信息。Memory Bank 特别适用于与 AI 代理合作时，因为它包含结构化的信息，涵盖：
...
在与这个项目的 AI 代理合作时，Memory Bank 作为一个全面的知识库，帮助代理理解：
...
AI 代理可以阅读这些文件，快速熟悉项目背景，并对代码变更和实现做出更明智的决策。

## 测试

详细文档可在提供的 [链接](./test-utils/docs/README.md) 中获取。