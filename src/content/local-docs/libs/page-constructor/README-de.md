...
Beim Erstellen von Seiten wird ein komponentenbasierter Ansatz verwendet: Eine Seite wird aus einem Satz vorgefertigter Blöcke aufgebaut, die in beliebiger Reihenfolge platziert werden können. Jeder Block hat einen bestimmten Typ und eine Reihe von Eingabedatenparametern.
...
Zuerst benötigen wir ein React-Projekt und einen Server. Zum Beispiel können Sie ein React-Projekt mit Vite und einem Express-Server erstellen oder eine Next.js-Anwendung – diese hat auf einmal Client- und Server-Seite.
...
interface PageConstructorProviderProps {
  ...
  location?: Location; //API des Browsers oder des Router-Verlaufs, die URL der Seite.
  analytics?: AnalyticsContextProps; // Funktion zur Behandlung von Analyseereignissen

  ssrConfig?: SSR; //Ein Flag, das anzeigt, dass der Code auf der Server-Seite ausgeführt wird.
...
Das Paket bietet eine Reihe von Server-Utilities zur Transformation Ihres Inhalts.
...
Im Hintergrund wird ein Paket verwendet, um Yandex Flavored Markdown in HTML umzuwandeln – `diplodoc/transform`, das daher auch in den Peer-Dependencies enthalten ist.
...
2. Erstellen Sie in Ihrem Code ein Objekt, bei dem der Block-Typ (String) als Schlüssel und die importierte Block-Komponente als Wert dient.
...
Um Mixins und Constructor-Stil-Variablen bei der Erstellung benutzerdefinierter Blöcke zu verwenden, fügen Sie einen Import in Ihre Datei hinzu:
...
Manchmal ist es notwendig, dass ein Block sich selbst basierend auf zu ladenden Daten rendert. In diesem Fall kommen ladbare Blöcke zum Einsatz.
...
Der Page Constructor verwendet das `Bootstrap`-Grid und seine Implementierung basierend auf React-Komponenten, die Sie auch in Ihrem eigenen Projekt verwenden können (einschließlich separat vom Constructor).
...
2. Fügen Sie den Namen des Blocks oder Sub-Blocks zum Enum `BlockType` oder `SubBlockType` hinzu und beschreiben Sie seine Eigenschaften in der Datei `src/models/constructor-items/blocks.ts` oder `src/models/constructor-items/sub-blocks.ts` auf ähnliche Weise wie bei den bestehenden.
...
4. Fügen Sie die neue Komponente oder den neuen Block zur Zuordnung in `src/constructor-items.ts` hinzu.
...
1. Definieren Sie in der Datei `models/blocks.ts` den Typ der jeweiligen Block-Eigenschaft mit dem generischen `ThemeSupporting<T>`, wobei `T` der Typ der Eigenschaft ist.

2. In der Datei mit der React-Komponente des Blocks erhalten Sie den Wert der thematisierten Eigenschaft über `getThemedValue` und den `useTheme`-Hook (siehe Beispiele im Block `MediaBlock.tsx`).
...
Der `page-constructor` ist eine auf `uikit` basierende Bibliothek, und wir verwenden eine Instanz von `i18n` aus uikit. Um die Internationalisierung einzurichten, müssen Sie lediglich `configure` aus uikit verwenden:
...
Um Karten zu verwenden, geben Sie den Kartentyp, `scriptSrc` und `apiKey` im Feld `mapContext` des `PageConstructorProvider` an.
...
Ein Event-Objekt hat nur ein erforderliches Feld – `name`. Es enthält zudem vordefinierte Felder, die dabei helfen, komplexe Logik zu verwalten. Zum Beispiel kann `counter.include` nützlich sein, um ein Event in einen bestimmten Counter zu senden, falls mehrere Analytics-Systeme in einem Projekt verwendet werden.
...
Es ist möglich, für ein Event festzulegen, an welches Analytics-System es gesendet werden soll.
...
Übergeben Sie den Wert für `context`, um den Ort im Projekt zu definieren, an dem das Event ausgelöst wird.

Verwenden Sie den unten stehenden Selektor oder erstellen Sie eine Logik, die den Anforderungen des Projekts entspricht.
...
Mehrere vordefinierte Event-Typen werden verwendet, um automatisch konfigurierte Events zu markieren. Diese Typen eignen sich z. B. zum Filtern von Standard-Events.
...
3. BREAKING CHANGE: Ein Commit mit dem Footer BREAKING CHANGE: oder einem ! nach dem Typ/Scope führt eine breaking API-Änderung ein (das entspricht MAJOR in Semantic Versioning). Ein BREAKING CHANGE kann in Commits beliebiger Typen vorkommen.
...
Alle Details finden Sie [hier](https://www.conventionalcommits.org/en/v1.0.0/).

Sobald Ihre Pull-Request von den Code-Ownern genehmigt wurde und alle Checks bestanden sind, gehen Sie wie folgt vor:
...
2. Squaschen und mergen Sie Ihren PR (Wichtig, um eine neue Version mit GitHub Actions zu releasen).
3. Warten Sie, bis der Robot einen PR mit der neuen Paketversion und Infos zu Ihren Änderungen in der CHANGELOG.md erstellt. Den Prozess sehen Sie im [Actions-Tab](https://github.com/gravity-ui/page-constructor/actions).
4. Überprüfen Sie Ihre Änderungen in der CHANGELOG.md und genehmigen Sie den PR des Robots.
5. Squaschen und mergen Sie den PR. Den Release-Prozess sehen Sie im [Actions-Tab](https://github.com/gravity-ui/page-constructor/actions).

### Alpha-Versionen releasen
...
4. Sie sehen auch ein Feld für die manuelle Version. Bei der ersten Alpha-Release aus Ihrem Branch lassen Sie es leer. Nach dem ersten Release müssen Sie die neue Version manuell setzen, da wir package.json nicht ändern (der Branch könnte sonst sehr schnell ablaufen). Verwenden Sie das Präfix `alpha` in Ihrer manuellen Version, andernfalls erhalten Sie einen Fehler.
...
5. Sie können das so oft wiederholen, wie Sie möchten. Wenn Sie bereit sind, die finale Major-Version ohne Beta-Tag zu releasen, erstellen Sie einen PR vom Branch `beta` zum Branch `main`. Beachten Sie, dass es normal ist, wenn Ihre Paketversion das Beta-Tag hat. Der Robot erkennt das und passt es korrekt an. `3.0.0-beta.0` wird zu `3.0.0`.

### Release-Prozess für vorherige Major-Versionen

Falls Sie eine neue Version in einer vorherigen Major nach dem Commit zum Main releasen möchten, gehen Sie wie folgt vor:

1. Aktualisieren Sie den relevanten Branch. Die Namen der Branches für vorherige Major-Releases lauten:
...
4. Erstellen Sie einen PR, lassen Sie ihn genehmigen und mergen Sie in den vorherigen Major-Release-Branch.
5. Squaschen und mergen Sie Ihren PR (Wichtig, um eine neue Version mit GitHub Actions zu releasen).
6. Warten Sie, bis der Robot einen PR mit der neuen Paketversion und Infos zu Ihren Änderungen in der CHANGELOG.md erstellt. Den Prozess sehen Sie im [Actions-Tab](https://github.com/gravity-ui/page-constructor/actions).
7. Überprüfen Sie Ihre Änderungen in der CHANGELOG.md und genehmigen Sie den PR des Robots.
8. Squaschen und mergen Sie den PR. Den Release-Prozess sehen Sie im [Actions-Tab](https://github.com/gravity-ui/page-constructor/actions).
...
Dieses Projekt umfasst eine umfassende **Memory Bank** – eine Sammlung von Markdown-Dokumentationsdateien, die detaillierte Informationen über die Architektur des Projekts, die Komponenten und Nutzungsmuster liefern. Die Memory Bank ist besonders nützlich beim Arbeiten mit KI-Agenten, da sie strukturierte Informationen zu enthält:
...
- `projectbrief.md` - Grunddokument mit Kernanforderungen
...
- `storybookComponents.md` - Storybook-Integrationsdetails

### Für KI-Agenten

Beim Arbeiten mit KI-Agenten an diesem Projekt dient die Memory Bank als umfassende Wissensbasis, die den Agenten hilft zu verstehen:
...
KI-Agenten können diese Dateien lesen, um schnell mit dem Projektkontext vertraut zu werden und fundiertere Entscheidungen über Codeänderungen und Implementierungen zu treffen.
