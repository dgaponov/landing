...
`Page-constructor` est une bibliothèque pour rendre des pages web ou leurs parties en se basant sur des données `JSON` (le support du format `YAML` sera ajouté ultérieurement).
...
Tout d'abord, nous avons besoin d'un projet React et d'un serveur quelconque. Par exemple, vous pouvez créer un projet React avec Vite et un serveur Express, ou bien une application Next.js – qui dispose à la fois d'un côté client et serveur.
...
Insérez le `Page Constructor` dans la page. Pour qu'il fonctionne correctement, il doit être enveloppé dans un `PageConstructorProvider` :
...
C'était l'exemple le plus simple d'intégration. Pour que le balisage YFM fonctionne, vous devez traiter le contenu côté serveur et le recevoir côté client.
...
interface PageConstructorProps {
  ...
  shouldRenderBlock?: ShouldRenderBlock; // Une fonction appelée lors du rendu de chaque bloc et qui permet de définir des conditions pour son affichage.
  ...
  isBranded?: boolean; // Si true, ajoute un pied de page qui lie vers https://gravity-ui.com/. Essayez le composant BrandFooter pour plus de personnalisation.
}
...
3. Passez l'objet que vous avez créé au paramètre `custom.blocks`, `custom.headers` ou `custom.subBlocks` du composant `PageConstructor` (`custom.headers` spécifie les en-têtes de blocs à rendre séparément au-dessus du contenu général).
...
export interface LoadableConfigItem {
  ...
  component: React.ComponentType; // composant à passer les données chargées
}
...
8. Ajoutez un modèle de données de bloc à `src/editor/data/templates/`, le nom du fichier doit correspondre au type de bloc.
9. (optionnel) Ajoutez une icône d'aperçu de bloc à `src/editor/data/previews/`, le nom du fichier doit correspondre au type de bloc.

### Thèmes

`PageConstructor` vous permet d'utiliser des thèmes : vous pouvez définir des valeurs différentes pour les propriétés individuelles de blocs en fonction du thème sélectionné dans l'application.
...
Un objet d'événement ne comporte qu'un seul champ obligatoire : `name`. Il possède également des champs prédéfinis qui servent à gérer une logique complexe. Par exemple, `counter.include` permet d'envoyer un événement vers un compteur spécifique si plusieurs systèmes d'analyse sont utilisés dans un projet.
...
Pour Vite, vous devez installer le plugin `vite-plugin-dynamic-import` et configurer le fichier de configuration afin que les imports dynamiques fonctionnent correctement.
...
Lorsque vous obtenez l'approbation de votre pull request par les propriétaires du code et que tous les contrôles sont passés, veuillez procéder comme suit :
...
3. Vous verrez un bouton "Run workflow" sur le côté droit. Ici, vous pouvez choisir la branche.
4. Vous verrez également un champ pour la version manuelle. Si c'est la première fois que vous publiez une alpha depuis votre branche, ne remplissez rien ici. Après la première publication, vous devrez définir la nouvelle version manuellement car nous ne modifions pas package.json au cas où la branche expirerait rapidement. Utilisez le préfixe `alpha` dans votre version manuelle, sinon vous obtiendrez une erreur.
...
5. Vous pouvez répéter cela autant de fois que nécessaire. Lorsque vous êtes prêt à publier la dernière version majeure sans le tag beta, créez une PR de la branche `beta` vers la branche `main`. Notez qu'il est normal que la version de votre package comporte un tag beta. Le robot le sait et le gère correctement. `3.0.0-beta.0` deviendra `3.0.0`.
...
1. Mettez à jour la branche nécessaire. Les noms des branches de publication des versions majeures précédentes sont :
   1. `version-1.x.x/fixes` - pour la majeure 1.x.x
   2. `version-2.x.x` - pour la majeure 2.x.x
...
Ce projet inclut une **Memory Bank** complète - une collection de fichiers de documentation Markdown qui fournissent des informations détaillées sur l'architecture du projet, ses composants et ses modèles d'utilisation. La Memory Bank est particulièrement utile lors du travail avec des agents IA, car elle contient des informations structurées sur :
...
- **Architecture système** : Modèles techniques et décisions de conception
- **Avancement du développement** : État actuel et détails d'implémentation

### Utilisation de la Memory Bank

La Memory Bank se trouve dans le répertoire `memory-bank/` et est composée de fichiers Markdown ordinaires qui peuvent être lus comme n'importe quelle autre documentation :

- `projectbrief.md` - Document de base avec les exigences principales
...
- `progress.md` - État d'implémentation et problèmes connus
- `usage/` - Documentation d'utilisation spécifique aux composants
- `storybookComponents.md` - Détails sur l'intégration de Storybook
...
- Les flux de travail de développement et les meilleures pratiques
- L'état d'implémentation actuel et les étapes suivantes

Les agents IA peuvent lire ces fichiers pour se mettre rapidement au courant du contexte du projet et prendre des décisions plus éclairées concernant les modifications de code et les implémentations.

## Tests

Une documentation complète est disponible à l'adresse indiquée [ici](./test-utils/docs/README.md).